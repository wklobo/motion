...
#define __TSL2561_DEBUG__ false
...
void* tsl2561_init()
{
  ...
  DEBUG(">> %s()#%d - _tsl = '%p'\n", __FUNCTION__, __LINE__, _tsl);

  tsl2561_t* tsl = TO_TSL(_tsl);
  DEBUG(">> %s()#%d - tsl = '%p'\n", __FUNCTION__, __LINE__, tsl);

  tsl->address = TLS2561ADDR;
  ...
  // ----------------------
  tsl->i2c_device = (char*) malloc(strlen(I2C_DEVICE) * sizeof(char));
  if(tsl->i2c_device == NULL) {
    DEBUG("error: malloc returns NULL pointer!\n");
  ...
  // copy string
  strcpy(tsl->i2c_device, I2C_DEVICE);
...
{
  return tsl2561_write_byte_data(_tsl,
                      TSL2561_CMD_BIT | TSL2561_REG_CTRL, TSL2561_CTRL_PWR_ON);
}
//***************************************************************************//
/**
...
{
  return tsl2561_write_byte_data(_tsl,
                     TSL2561_CMD_BIT | TSL2561_REG_CTRL, TSL2561_CTRL_PWR_OFF);
}
//***************************************************************************//
/**
...
void tsl2561_read (void *_tsl, int *broadband, int *ir)
{
  ...
  *broadband = tsl2561_read_word_data(_tsl,
                     TSL2561_CMD_BIT | TSL2561_WORD_BIT | TSL2561_REG_CH0_LOW);
  *ir = tsl2561_read_word_data(_tsl,
                     TSL2561_CMD_BIT | TSL2561_WORD_BIT | TSL2561_REG_CH1_LOW);
